...
class CoinListViewModel @Inject constructor(
    ...
    private fun getCoins() {
        getCoinsUseCase().onEach { result ->
            when (result) {
                is Resource.Error -> _state.value = CoinListState(error = result.errorMessage ?: R.string.error_unexpected)
                is Resource.Loading -> _state.value = CoinListState(isLoading = true)
                is Resource.Success -> _state.value = CoinListState(coins = result.data ?: emptyList())
            }
